...
import static org.jeecg.modules.common.utils.DateTimeUtils.*;

@Slf4j
public class OrderNoRule implements IFillRuleHandler {
    @Override
    public Object execute(JSONObject params, JSONObject formData) {
        String prefix = "S"; // 默认前缀
        if (params != null && params.containsKey("prefix")) {
            String paramPrefix = params.getString("prefix");
            if (StringUtils.isNotBlank(paramPrefix)) {
                prefix = paramPrefix;
            }
        }
        return SerialNoUtils.generate(prefix);
    }
//    //@Autowired
