...
import static org.jeecg.modules.mdm.constant.ItemTypeConstants.PRODUCT;
/**
 * @Description: 产品信息
 * @Author: jeecg-boot
 * @Date:   2025-05-18
...
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product> implements IProductService {
    ...
    @Override
    public Item buildItemFromMaterial(Product product) {
        Item item = new Item();
        item.setItemId(product.getId());
        item.setCode(product.getProductCode());
        item.setName(product.getProductName());
        item.setItemType(PRODUCT);
        item.setSpec(product.getProductSpec());
        item.setUnit("kg");
        item.setDelFlag(product.getDelFlag());
        item.setIsActive(product.getStatus());
        item.setSysOrgCode(product.getSysOrgCode());
        return item;
    }
