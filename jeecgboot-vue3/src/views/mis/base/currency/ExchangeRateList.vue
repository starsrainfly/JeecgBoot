<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <ExchangeRateModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import {exchangeRateColumns} from './Currency.data';
  import {exchangeRateList, exchangeRateDelete, exchangeRateDeleteBatch, exchangeRateExportXlsUrl, exchangeRateImportUrl } from './Currency.api';
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        ...
        importConfig: {
            url: ()=>{
                return exchangeRateImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    watch(mainId, () => {
         searchInfo['currencyId'] = unref(mainId);
         reload();
      }
    );
    ...
    async function handleDelete(record) {
        await exchangeRateDelete({id: record.id}, handleSuccess);
    }
    ...
    async function batchHandleDelete() {
        await exchangeRateDeleteBatch({ids: selectedRowKeys.value}, handleSuccess)
    }
