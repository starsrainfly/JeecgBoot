    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
      <a-tab-pane tab="生产订单明细" key="productionOrderDetail" :forceRender="true">
        <JVxeTable
          ...
          v-if="productionOrderDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,productionOrderDetailColumns} from '../ProductionOrder.data';
  import {saveOrUpdate,productionOrderDetailList} from '../ProductionOrder.api';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/mes/productionOrder/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(productionOrderDetailList, {id: data.id}, productionOrderDetailTable, ()=>{
          productionOrderDetailTable.show = true;
        });
        //默认是禁用
