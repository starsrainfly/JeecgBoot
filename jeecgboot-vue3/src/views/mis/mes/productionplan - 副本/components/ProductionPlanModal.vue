...
// 【关键调试】确认回填后表单内部状态
          setTimeout(async () => {
            ...
            console.log('【调试】编辑模式表单值:', JSON.stringify(vals));
            console.log('【调试】productId:', vals?.productId);
            console.log('【调试】productCode:', vals?.productCode);
            console.log('【调试】plannedQty:', vals?.plannedQty);
          }, 200);
    ...
    async function requestAddOrEdit(values) {
      console.log('【requestAddOrEdit】values:', JSON.stringify(values));

      // 强制获取表单值
      const currentForm = await getFieldsValue();
      console.log('【requestAddOrEdit】getFieldsValue:', JSON.stringify(currentForm));
        try {
          // 最终校验 - 实时获取最新值
          const result = await validateDetailAllocatedQty();
          if (!result.valid) {
