...
<script lang="ts">
  ...
  import {getBpmFormSchema,costCalcDetailColumns} from '../CostCalc.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/scm/costCalc/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(costCalcDetailList, {id: data.id}, costCalcDetailTable, ()=>{
          costCalcDetailTable.show = true;
        });
        //默认是禁用
