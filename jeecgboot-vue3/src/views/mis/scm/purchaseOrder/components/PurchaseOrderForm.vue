    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
      <a-tab-pane tab="采购明细" key="purchaseOrderDetail" :forceRender="true">
        <JVxeTable
          ...
          v-if="purchaseOrderDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,purchaseOrderDetailColumns} from '../PurchaseOrder.data';
  import {saveOrUpdate,purchaseOrderDetailList} from '../PurchaseOrder.api';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      function classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          purchaseOrderDetailList: allValues.tablesValue[0].tableData,
        }
      }
      ...
      const queryByIdUrl = '/scm/purchaseOrder/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(purchaseOrderDetailList, {id: data.id}, purchaseOrderDetailTable, ()=>{
          purchaseOrderDetailTable.show = true;
        });
        //默认是禁用
