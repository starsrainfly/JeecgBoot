    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
      <a-tab-pane tab="收款明细" key="receiptOrderDetail" :forceRender="true">
        <JVxeTable
          ...
          v-if="receiptOrderDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,receiptOrderDetailColumns} from '../ReceiptOrder.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/scm/receiptOrder/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(receiptOrderDetailList, {id: data.id}, receiptOrderDetailTable, ()=>{
          receiptOrderDetailTable.show = true;
        });
        //默认是禁用
