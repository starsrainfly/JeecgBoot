...
<script lang="ts" setup>
    ...
    //  * 处理单元格变化事件 - 实时计算
    //  */
    // function handleValueChange1(event) {
    //   const { type, value, oldValue, row, column, rowIndex, $table } = event;
    //   const field = column.field || column.property;
    //
    //   console.log('值变化:', field, '新值:', value, '行:', row);
    //
    //   // 获取表格实例
    ...
    //
    //   // 1. 申请数量变化时，自动填充到实收数量（如果实收为空）
    //   if (field === 'applyQty') {
    //     const applyQty = parseFloat(value || 0);
    //
    //     // 检查实收数量是否为空
    //   //  if (row.actualQty === undefined || row.actualQty === null || row.actualQty === '') {
    ...
    //
    //   // 2. 当相关字段变化时，重新计算金额
    //   if (['applyQty', 'actualQty', 'unitPrice', 'exchangeRate'].includes(field)) {
    //     // 如果当前变化的是applyQty且actualQty被设置了，使用新值
    //     const actualQty = field === 'applyQty' && valuesToSet.actualQty !== undefined
    //       ? valuesToSet.actualQty
    ...
    //   }
    // }
    // async function handleValueChange2({ row, column, cellValue, oldValue }) {
    ...
    //   if (needRefresh) {
    //     await nextTick(() => {
    //       // 方式1: 使用JVxeTable的refresh方法
    //       if (stockInDetail.value && typeof stockInDetail.value.refresh === 'function') {
    //         stockInDetail.value.refresh();
    //       }
    //       // 方式2: 如果有updateData方法
    //       else if (stockInDetail.value && typeof stockInDetail.value.updateData === 'function') {
    //         stockInDetail.value.updateData([row]);
    //       }
    //       // 方式3: 强制刷新整个表格数据
    //       else {
    //         // 临时保存当前数据，然后重新赋值以触发刷新
    //         const currentData = [...stockInDetailTable.dataSource];
    //         stockInDetailTable.dataSource = [];
    //         nextTick(() => {
    //           stockInDetailTable.dataSource = currentData;
    //         });
    //       }
    //     });
    //   }
