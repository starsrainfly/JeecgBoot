...
<script lang="ts">
  ...
  import {getBpmFormSchema,stockOutDetailColumns} from '../MaterialOutApprove.data';
  import {saveOrUpdate,stockOutDetailList} from '../MaterialOutApprove.api';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/wms/stockOut/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(stockOutDetailList, {id: data.id}, stockOutDetailTable, ()=>{
          stockOutDetailTable.show = true;
        });
        //默认是禁用
